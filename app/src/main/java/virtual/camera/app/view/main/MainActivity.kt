...
class MainActivity : LoadingActivity() {
    ...
    fun installApk(source: String) {
        fragmentList.getOrNull(currentUser)?.installApk(source)
    }
    
    fun cloneApp(packageName: String) {
        // Clone app functionality - create a copy in next user slot
        val nextUserId = currentUser + 1
        if (nextUserId < fragmentList.size) {
            fragmentList[nextUserId].installApk(packageName)
            ToastUtils.showToast("Cloning app to User $nextUserId")
        } else {
            ToastUtils.showToast("No more user slots available")
        }
    }
