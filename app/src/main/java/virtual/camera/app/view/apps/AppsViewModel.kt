...
class AppsViewModel(private val repo: AppsRepository) : BaseViewModel() {
    ...
    val installResultLiveData = resultLiveData
    val unInstallResultLiveData = resultLiveData
    val launchResultLiveData = MutableLiveData<Boolean>()
    ...
    fun getAppList(userId: Int) = getInstalledApps(userId)
    
    fun updateSortList(userID: Int, dataList: List<AppInfo>) = updateApkOrder(userID, dataList)
